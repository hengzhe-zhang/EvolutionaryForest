...
class MutationTransformer(nn.Module):
    def __init__(
        ...
        self.embedding = nn.Embedding(
            embedding_class, embedding_size, padding_idx=padding_idx
        )
    ...
    def forward(self, batch_x, batch_context):
        ...
        torch_output = self.transformer_encoder.forward(
            torch.swapdims(torch_input_embedding, 0, 1)
        )
        candidate_primitives = self.embedding(
            torch.IntTensor([i for i in range(self.embedding_class)]).to(device)
        )
        torch_output = (
            torch_output[:number_of_primitives, :, :] @ candidate_primitives.T
        )
...
def model_training(full_dataset, X, parameters=None):
    ...
    training_data, test_dataset = torch.utils.data.random_split(
        full_dataset, [train_size, test_size]
    )
    # training_data = full_dataset[test_size:]
    # test_dataset = full_dataset[:test_size]
    torch_input, torch_context, torch_target = training_data_process(
        training_data, primitive_encoder
    )
    torch_input_test, torch_context_test, torch_target_test = training_data_process(
        test_dataset, primitive_encoder
    )
    number_of_primitives = len(primitive_encoder.classes_)
    model = MutationTransformer(
        embedding_class=number_of_primitives + 1,
    ...
    for iter in range(20000):
        ...
        training_accuracy_nn, training_accuracy_random = calculate_accuracy(
        ...
        testing_accuracy_nn, testing_accuracy_random = calculate_accuracy(
            number_of_primitives,
            pset,
            test_dataset,
            torch_input_test,
...
def calculate_accuracy(
    ...
    for id, item in enumerate(label):
        ...
        old_pearson = pearsonr(
            single_tree_evaluation(ind, pset, context_x[id].T), context_y[id]
        )[0]
        ...
        for node_id, a in enumerate(ind):
            if isinstance(a, gp.Primitive):
                ...
                if item[current_pos] < len(primitive_encoder.classes_):
                    ...
                    new_node = pset.mapping[
                        primitive_encoder.classes_[item[current_pos]]
                    ]
        ...
        for node_id, a in enumerate(ind):
            if isinstance(a, gp.Primitive):
                # ensure it is a valid replacement
                all_primitives = list(
                    filter(lambda x: x.arity == a.arity, pset.primitives[object])
                )
    ...
    for c in range(torch_output_test.shape[1]):
        ...
        print(
            ...
            torch.sum(
                (
                    torch.argmax(torch_output_test[:, c], dim=1)
                    == torch_target_test[:, c]
    ...
    total_accuracy = torch.sum(
        (torch.argmax(torch_output_test, dim=2) == torch_target_test)[
...
def training_data_process(training_data, primitive_encoder):
        ...
        if number_of_terminals + 1 - context.shape[1] > 0:
            context = torch.cat(
                [
                    context,
                    torch.zeros(
                        (context.shape[0], number_of_terminals + 1 - context.shape[1])
                    ),
                ],
                dim=1,
            )
    ...
    torch_input = pad_sequence(
        torch_input, padding_value=len(primitive_encoder.classes_)
    ).T
    torch_target = pad_sequence(
        torch_target, padding_value=len(primitive_encoder.classes_)
    ).T
...
def get_primitive_set(X):
    ...
    primitive_encoder.fit(
        [
            p.name
            for p in filter(
                lambda x: isinstance(x, gp.Primitive), pset.primitives[object]
            )
        ]
    )
...
def training_data_generation(X, y):
    ...
    for _ in range(50000):
        ...
        while True:
            for node_id, node in enumerate(tree):
                if isinstance(node, gp.Primitive):
                    all_primitives = list(
                        filter(lambda x: x.arity == node.arity, pset.primitives[object])
                    )
                    new_node = random.choice(all_primitives)
        ...
        if np.abs(pearsonr(result_b, y_sample)[0]) > 10 * np.abs(
            pearsonr(result_a, y_sample)[0]
        ):
            ...
            print(
                np.abs(pearsonr(result_a, y_sample)[0]),
                np.abs(pearsonr(result_b, y_sample)[0]),
            )
            training_data.append((tree_a, tree_b, used_data))
        elif np.abs(pearsonr(result_a, y_sample)[0]) > 10 * np.abs(
            pearsonr(result_b, y_sample)[0]
        ):
            print(
                np.abs(pearsonr(result_a, y_sample)[0]),
                np.abs(pearsonr(result_b, y_sample)[0]),
...
if __name__ == "__main__":
    ...
    training_accuracy, testing_accuracy = model_training(
        training_data,
        X,
        {
