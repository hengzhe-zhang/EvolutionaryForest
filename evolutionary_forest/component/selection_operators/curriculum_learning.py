...
def adaptive_tier_lexicase_selection(
    population,
    num_selected,
    difficulty_metric="variance",
    generation_info=None,
    elite_tier_sampling=True,
):
    """
    Adaptive Tier Lexicase Selection: Combines the strengths of TierLexicase and
    AdaptiveEpsilon approaches with elite tier sampling.

    Key improvements:
    1. Precision-targeted epsilon scaling per tier
    2. Balanced tier sampling with slight preference for medium-difficulty cases
    3. Optional elite tier sampling to boost exploitation in later generations
    4. Adaptive case ordering based on performance-driven metrics
    """
    selected = []
    ...
    # Create tier sampling weights - more balanced than previous implementations
    # Medium cases always get slight preference (40%)
    tier_weights = {
        0: max(0.2, 0.4 - 0.2 * gen_progress),  # Easy: 40% → 20%
        1: 0.4,  # Medium: constant 40%
        2: min(0.4, 0.2 + 0.2 * gen_progress),  # Hard: 20% → 40%
    }

    # Refined epsilon scaling per tier
    epsilon_scales = {
        0: 1.1,  # Easy: slightly higher tolerance
        1: 1.0,  # Medium: standard tolerance
        2: max(0.7, 0.9 - 0.2 * gen_progress),  # Hard: tightening tolerance over time
    }

    # Track tier-specific performance for elite sampling
    if elite_tier_sampling and gen_progress > 0.5:
        # Calculate mean error per tier for each individual
        tier_performance = {}
        for i, ind in individual_map.items():
            tier_performance[i] = {}
            for tier, cases in precomputed_tiers.items():
                if cases:  # Check if the tier has any cases
                    tier_errors = [case_values[i, case_idx] for case_idx in cases]
                    tier_performance[i][tier] = np.mean(tier_errors)

    for _ in range(num_selected):
        # Initial ordering of cases
        ordered_cases = []

        # Apply elite tier sampling in later generations (optional)
        if elite_tier_sampling and gen_progress > 0.5:
            # Find best tier for each remaining individual
            best_tiers = {}
            for i in individual_map.keys():
                if tier_performance[i]:  # Check if we have performance data
                    best_tier = min(tier_performance[i], key=tier_performance[i].get)
                    if best_tier not in best_tiers:
                        best_tiers[best_tier] = []
                    best_tiers[best_tier].append(i)

            # Add a few cases from each individual's best tier
            for tier, individuals in best_tiers.items():
                if tier in precomputed_tiers and len(precomputed_tiers[tier]) > 0:
                    # Sample cases from this tier with slightly higher probability
                    sample_size = min(3, len(precomputed_tiers[tier]))
                    tier_cases = random.sample(precomputed_tiers[tier], sample_size)
                    ordered_cases.extend(tier_cases)

        # Add remaining cases based on tier weights
        for tier, cases in precomputed_tiers.items():
            if not cases:
                continue
            ...
            num_to_sample = max(1, int(len(shuffled_cases) * tier_weights[tier]))

            # Don't add duplicates
            new_cases = [
                c for c in shuffled_cases[:num_to_sample] if c not in ordered_cases
            ]
            ordered_cases.extend(new_cases)

        # Final shuffle for stochasticity
        random.shuffle(ordered_cases)

        candidates = list(individual_map.keys())

        for iteration, case_idx in enumerate(ordered_cases, start=1):
            # Get the tier for this case
            tier = next(
                (t for t, cases in precomputed_tiers.items() if case_idx in cases), 1
            )

            # Compute threshold using MAD with tier-specific scaling
            errors = case_values[candidates, case_idx]
            median_error = np.median(errors)
            mad = np.median(np.abs(errors - median_error))

            # Apply tier-specific epsilon scaling
            epsilon = mad * epsilon_scales[tier]
