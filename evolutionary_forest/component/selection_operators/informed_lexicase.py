...
def novel_selection_plus(population, k=100, status={}):
    stage = np.clip(status.get("evolutionary_stage", 0.0), 0, 1)
    n = len(population)
    k = min(k, (n // 2) * 2)
    if k == 0:
        return []

    Y = np.array([ind.y for ind in population])
    X = np.array([ind.predicted_values for ind in population])
    ...
    full_mse = np.array([np.mean(ind.case_values) for ind in population])

    max_nodes = nodes.max() if nodes.max() > 0 else 1
    max_height = height.max() if height.max() > 0 else 1
    complexity = 0.5 * (nodes / max_nodes) + 0.5 * (height / max_height)
    complexity /= complexity.max() if complexity.max() > 0 else 1

    # Parameters shift with stage to adapt pressure
    alpha = 0.3 + 0.7 * stage
    n_subsets = 5 + int(stage * 2)
    subset_size = max(1, X.shape[1] // n_subsets)

    rng = np.random.default_rng(42)
    subsets = rng.choice(X.shape[1], size=(n_subsets, subset_size), replace=False)

    mse_subsets = np.mean((Y[:, subsets] - X[:, subsets]) ** 2, axis=2)
    spec_scores = 1 / (1 + mse_subsets)
    spec_scores = (spec_scores.T / (spec_scores.max(axis=1) + 1e-10)).T
    max_spec = spec_scores.max(axis=1)
    max_spec /= max_spec.max() + 1e-10

    fitness = 1 / (1 + full_mse)

    # Combine fitness, specialization and penalize complexity, stage-adaptive
    scores = (
        alpha * fitness + (1 - alpha) * max_spec - (0.1 + 0.05 * stage) * complexity
    )
    ...
    def normalize_rows(A):
        ...
        norm = np.linalg.norm(A_c, axis=1, keepdims=True) + 1e-10
        return A_c / norm

    norm_residuals = normalize_rows(residuals)
    norm_a_res = norm_residuals[parent_a_idx]

    complexity_penalty = (0.04 + 0.03 * stage) * complexity
    max_cand = min(10, n - 1)

    # Precompute residual correlation matrix for efficiency
    corr_mat = norm_residuals @ norm_a_res.T
    np.fill_diagonal(corr_mat, 1)

    selected_b_idx = []
    for i, a_i in enumerate(parent_a_idx):
        corr = corr_mat[:, i]
        corr[a_i] = 1  # exclude self
        score_b = -np.abs(corr) - complexity_penalty
        score_b[a_i] = -np.inf
        candidates = np.argpartition(score_b, -max_cand)[-max_cand:]
        best_b = candidates[np.argmax(score_b[candidates])]
        selected_b_idx.append(best_b)

    parent_b_idx = np.array(selected_b_idx)
    pop_arr = np.array(population)
    return [
        ind
        for pair in zip(pop_arr[parent_a_idx], pop_arr[parent_b_idx])
        for ind in pair
    ]
