...
def novel_selection_plus(population, k=100, status={}):
    ...
    # Normalize metrics
    err_norm = (errors - errors.min()) / (errors.ptp() + 1e-9)
    comp = nodes + 1.4 * heights
    comp_norm = (comp - comp.min()) / (comp.ptp() + 1e-9)
    res_std = np.std(residuals, axis=1)
    res_std_norm = (res_std - res_std.min()) / (res_std.ptp() + 1e-9)

    # Weights vary with stage: early favors error + diversity, late adds interpretability
    w_err = 0.7 + 0.3 * stage  # 0.7 鈫?1.0
    w_comp = 0.4 * (1 - stage)  # diminish complexity weight late
    w_div = 0.3 * (1 - stage)  # more diversity early

    fitness = w_err * err_norm + w_comp * comp_norm - w_div * res_std_norm

    # Candidate pool for parent A with diversity clustering by residual cosine similarity
    pool_size = min(3 * k // 4, n)
    candidates = np.argsort(fitness)[:pool_size]

    feats = residuals[candidates]
    feats = feats[:, :40] if feats.shape[1] > 40 else feats
    norm_feats = feats / (np.linalg.norm(feats, axis=1, keepdims=True) + 1e-9)
    ...
    for i in range(len(candidates)):
        if i in used:
            continue
        mem = set(np.where(sim[i] > thresh)[0])
        clusters.append(mem)
        used |= mem

    parent_a = []
    for c in clusters:
        c_idxs = [candidates[i] for i in c]
        best = min(c_idxs, key=lambda ix: fitness[ix])
        parent_a.append(population[best])
        if len(parent_a) >= k // 2:
            break
    if len(parent_a) < k // 2:
        selected_set = set(parent_a)
        for ix in np.argsort(fitness):
            ind = population[ix]
            if len(parent_a) >= k // 2:
                break
    ...
    # Parent B: complement pairs minimizing combined residual magnitude and complexity distance
    max_nodes_ptp = max(nodes.ptp(), 1)
    max_heights_ptp = max(heights.ptp(), 1)
    avg_err = np.mean(errors)
    max_tries = 20

    parent_b = []
    for a in parent_a:
        a_res = a.y - a.predicted_values
        a_n, a_h = len(a), a.height
        best_b, best_score = None, np.inf
        tries = 0
        while tries < max_tries:
            tries += 1
            b = population[np.random.randint(n)]
            if b is a:
                continue
            b_res = b.y - b.predicted_values
            comp_dist = (
                abs(len(b) - a_n) / max_nodes_ptp
                + abs(b.height - a_h) / max_heights_ptp
            ) / 2
            score = np.mean(np.abs(a_res + b_res)) + 0.14 * comp_dist
            if score < best_score:
                best_score, best_b = score, b
            if best_score < 0.5 * avg_err:
                break
        parent_b.append(best_b if best_b else a)
