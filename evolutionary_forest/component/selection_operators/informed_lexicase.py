...
def novel_selection_plus(population, k=100, status={}):
    evo_stage = np.clip(status.get("evolutionary_stage", 0.5), 0, 1)
    n = len(population)
    half_k = k // 2
    ...
    residuals = np.array([ind.y - ind.predicted_values for ind in population])
    # Complexity: normalized sum of nodes and height
    complexities = np.array([len(ind) + ind.height for ind in population], dtype=float)
    cmin, cptp = complexities.min(), complexities.ptp()
    complexities = (complexities - cmin) / (cptp + 1e-9)
    # Specialization: inverse std error normalized
    err_std = np.array([ind.case_values.std() for ind in population])
    spec = 1 - (err_std - err_std.min()) / (err_std.ptp() + 1e-9)

    # Normalize errors
    emin, eptp = errors.min(), errors.ptp()
    errors_norm = (errors - emin) / (eptp + 1e-9)

    # Residual similarity matrix (cosine similarity)
    r_norm = residuals / (np.linalg.norm(residuals, axis=1, keepdims=True) + 1e-12)
    sim_mat = r_norm @ r_norm.T
    np.fill_diagonal(sim_mat, 0)

    # Weights varying by evolutionary stage
    w_err = 0.7 * evo_stage + 0.3  # error importance grows over time
    w_spec = 0.5 * (1 - evo_stage)  # favor specialization early
    w_comp = 0.6 * (1 - evo_stage)  # complexity pressure decreases with stage
    w_div = 0.4 * (1 - evo_stage)  # diversity pressure early

    # Diversity as inverse mean similarity
    diversity = 1 - sim_mat.mean(axis=1)

    # Score for Parent A: combine normalized error, specialization, complexity, and diversity
    score_a = (
        -w_err * errors_norm + w_spec * spec - w_comp * complexities + w_div * diversity
    )
    # Probabilities with softmax scaling for numerical stability
    exp_score = np.exp(score_a - np.max(score_a))
    probs_a = exp_score / exp_score.sum()

    selected_a = []
    max_attempts = max(1000, 20 * half_k)
    attempts = 0
    while len(selected_a) < half_k and attempts < max_attempts:
        idx = np.random.choice(n, p=probs_a)
        # Ensure diversity among selected by max similarity threshold
        if all(sim_mat[idx, sel] < 0.9 for sel in selected_a):
            selected_a.append(idx)
        attempts += 1
    if len(selected_a) < half_k:
        selected_a += list(np.random.choice(n, half_k - len(selected_a), replace=True))
    selected_a = np.array(selected_a)

    # Select Parent B optimizing complementarity with paired A
    selected_b = []
    for ia in selected_a:
        combined_err = errors + errors[ia] - 2 * (1 - sim_mat[ia])
        combined_comp = complexities + complexities[ia]
        comp_div = 1 - sim_mat[ia]
        # Normalize combined error and complexity
        ce_min, ce_ptp = combined_err.min(), combined_err.ptp()
        cc_min, cc_ptp = combined_comp.min(), combined_comp.ptp()
        c_err_norm = (combined_err - ce_min) / (ce_ptp + 1e-9)
        c_comp_norm = (combined_comp - cc_min) / (cc_ptp + 1e-9)

        w_err_b = 0.75 * evo_stage + 0.25
        w_comp_b = 0.5 * (1 - evo_stage)
        w_div_b = 0.3

        comp_score = -w_err_b * c_err_norm - w_comp_b * c_comp_norm + w_div_b * comp_div
        comp_score[ia] = -np.inf  # exclude self
        comp_score[selected_a] = -np.inf  # exclude already selected A individuals
        jb = comp_score.argmax()
        selected_b.append(jb)
