...
class SHM:
    ...
    def semantic_hoist_mutation(
        self, population: List[MultipleGeneGP], adaptive_hoist_probability
    ):
    ...
    def hoist_mutation(
        ...
        if (
            hoist_frequency != 0
            and self.algorithm.current_gen % hoist_frequency != 0
        ...
        all_genes = [
            list(
                sorted(
                    [(k, getattr(g, "corr", 0), gid) for k, g in enumerate(gene)],
                    key=lambda x: (x[1], x[0]),
                    reverse=True,
                )
            )
            for gid, gene in enumerate(o.gene)
        ]

        dynamic_threshold = np.mean(
            list(map(lambda g: g[1], chain.from_iterable(all_genes)))
        )
        dynamic_threshold = dynamic_threshold * self.algorithm.bloat_control.get(
            "dynamic_threshold", 0
        )
        ...
        while len(new_gene) < self.algorithm.gene_num:
            ...
            for eid, gid in order:
                ...
                if hoist_probability is None:
                    hoist_probability = self.algorithm.bloat_control.get(
                        "hoist_probability", 1
                    )
                hoisted = False
                if random.random() < hoist_probability:
                    ...
                    if (
                        ...
                        and random.random() < dropout_probability
                    ):
                    ...
                    for c_index, candidate_id in enumerate(gene_list):
                        ...
                        if no_check or (
                            gene_str not in previous_gene
                            and getattr(gene[candidate_id[0]], "corr", 0)
                            > dynamic_threshold
                        ):
                            # never hoist a redundant gene or a irrelevant gene
                            previous_gene.add(gene_str)
                            new_gene.append(
                                hoistMutation(copy.deepcopy(gene), candidate_id[0])
                            )
                            hoisted = True
                    ...
                    if (
                        not hoisted
                        and self.algorithm.bloat_control.get(
                            "feature_reinitialization", False
                        )
                        and not no_check
                    ):
                        if shared_gene:
                    ...
                    if (
                        not hoisted
                        and self.algorithm.bloat_control.get("do_nothing", False)
                        and not no_check
                    ):
                    ...
                    if (
                        not hoisted
                        and self.algorithm.bloat_control.get("zero_tree", False)
                        and not no_check
                    ):
                    ...
                    if not hoisted and not iteratively_check:
                        ...
                        """
                        raise Exception(
                            f"There is no available operation for tree {gid}!"
                        )
                else:
                    new_gene.append(gene)
