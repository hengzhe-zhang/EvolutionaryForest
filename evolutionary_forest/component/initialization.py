...
def initialize_crossover_operator(
    self: "EvolutionaryForestRegressor", toolbox: TypedToolbox
):
    ...
    while len(result) < n:
        ...
        if successive_failures < 10:
            if str(s) not in generated:
                ...
                result.append(s)
                successive_failures = (
                    0  # Reset the counter if we successfully added a new value
                )
            else:
