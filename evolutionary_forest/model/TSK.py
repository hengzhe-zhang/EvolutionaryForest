...
class TSKFuzzySystem(BaseEstimator, RegressorMixin):
    ...
    def fit(self, X, y):
        ...
        self.centers_ = torch.nn.Parameter(
        ...
        self.sigmas_ = torch.nn.Parameter(
            torch.tensor(
                np.std(
                    X.numpy()
                    - kmeans.transform(X.numpy()).argmin(axis=1, keepdims=True),
                    axis=0,
                ),
                dtype=torch.float32,
            )
        ...
        for r in range(self.n_rules):
            ...
            weight = torch.nn.Parameter(
                torch.tensor(bias_and_weights, dtype=torch.float32)
            )
            self.regressors_.append(weight)

        # Optimizer (using SGD as an example)
        optimizer = optim.Adam(
            [self.centers_, self.sigmas_] + self.regressors_, lr=self.learning_rate
        )
    ...
    def _forward(self, X, training=False):
        ...
        for r, weight in enumerate(self.regressors_):
            ...
            if training and self.drop_rate > 0:
                drop_mask = torch.bernoulli(torch.tensor(self.drop_rate)).type(
                    torch.float32
                )
                firing_level = firing_level * drop_mask

            X_bias = torch.cat(
                (torch.ones(X.shape[0], 1, dtype=torch.float32), X), dim=1
            )
            rule_output = firing_level * torch.matmul(X_bias, weight)
            rule_outputs.append(rule_output)

        system_output = sum(rule_outputs) / sum(
            self._compute_firing_levels(X, r) for r in range(self.n_rules)
        )
    ...
    def _compute_firing_levels(self, X, rule_idx):
        return torch.prod(
            torch.exp(
                -torch.pow(X - self.centers_[rule_idx], 2)
                / (2 * torch.pow(self.sigmas_, 2))
            ),
            axis=1,
        )
...
if __name__ == "__main__":
    ...
    tsk = TSKFuzzySystem(
        n_rules=5, n_epochs=100, learning_rate=0.001, drop_rate=0, verbose=True
    )
