...
def avg_function_to_terminal_ratio(individual):
    return aggregate_trees(
        individual, function_to_terminal_ratio, aggregation="average"
    )
...
def overall_function_usage(individual):
    ...
    all_functions = [
        function for tree in individual for function in function_usage(tree).keys()
    ]
...
def overall_terminal_usage(individual):
    ...
    all_terminals = [
        terminal for tree in individual for terminal in terminal_usage(tree).keys()
    ]
...
def extract_global_vocab(individuals):
    ...
    for ind in individuals:
        all_functions.update(
            [func for tree in ind for func in function_usage(tree).keys()]
        )
        all_terminals.update(
            [term for tree in ind for term in terminal_usage(tree).keys()]
        )
...
def vectorize_usage(individual, all_functions, all_terminals):
    ...
    func_vector = [
        func_usage.get(func, 0) for func in all_functions if func in func_usage
    ]
    term_vector = [
        term_usage.get(term, 0) for term in all_terminals if term in term_usage
    ]
