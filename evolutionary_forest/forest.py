...
from evolutionary_forest.component.selection_operators.informed_lexicase import (
    ...
    novel_selection_flux,
    novel_selection_nova,
)
...
from evolutionary_forest.strategies.auto_sam import auto_sam_scaling
...
class EvolutionaryForestRegressor(RegressorMixin, TransformerMixin, BaseEstimator):
    ...
    def __init__(
        ...
        **params,
        ...
        if normalize is True:
            ...
            self.y_scaler = StandardScaler()
        elif normalize == "Robust+Clip":
            self.x_scaler = RealMLPPreprocessor()
            self.y_scaler = StandardScaler()
        ...
        if self.base_learner in [
            ...
            if self.ridge_alphas is None:
                ...
                if len(np.unique(self.y)) <= 50:
                    ...
                    ridge = [0.1, 1, 10]
            elif self.ridge_alphas == "Auto-Compact":
                # Use Instances*Features < 500 as threshold
                instances_features = self.X.shape[0] * self.X.shape[1]
                if instances_features < 500:
                    ridge = [1e1, 1e2, 1e3]
                else:
                    ridge = [0.1, 1, 10]
            else:
    ...
    def selection_operator_initialization(self, toolbox):
        ...
        if isinstance(self.select, Selection):
            ...
            "RLS-A-Neg",
            "RLS-A-Both",
        ] or self.select.startswith("RLS"):
            # Extract temperature from selection name (e.g., "RLS-A-0.5" -> 0.5)
            temperature = extract_temperature_from_name(self.select)
            # Remove temperature suffix from self.select (e.g., "RLS-A-0.5" -> "RLS-A")
            self.select = re.sub(r"-[\d.]+$", "", self.select)
            rl_config = RLConfig(temperature=temperature)
            self.rl_selection = ParentSelectorRL(len(self.X), cfg=rl_config)
            ...
            toolbox.register("select", select_lexicase_random_second)
        ...
        elif self.select == "NovelSelection-Flux":
            toolbox.register("select", novel_selection_flux)
        elif self.select == "NovelSelection-Nova":
            toolbox.register("select", novel_selection_nova)
    ...
    def callback(self):
        ...
        if self.mutation_configuration.pool_based_addition:
            ...
            if "UniqueFitnessInPop" in self.log_item:
                self.semantic_lib_log.unique_fitness_in_pop_update(self.pop)
            if "SemanticLibUsage" in self.log_item:
                self.semantic_lib_log.semantic_lib_usage_update(self.tree_pool)
            if "BestTreeSize" in self.log_item:
                self.semantic_lib_log.update_best_tree_size(self.hof)
    ...
    def complexity(self):
        ...
        for h in self.hof:
            ...
            if isinstance(learner, DecisionTreeRegressor):
                ...
                for weight in learner.knn_model_.weights:
